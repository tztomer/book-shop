  ...
  */
  ...
  function findProp(node, name, dynamicOnly = false, allowEmpty = false) {
    for (let i = 0; i < node.props.length; i++) {
      ...
      } else if (p.name === "bind" && (p.exp || allowEmpty) && isStatibookgOf(p.arg, name)) {
        return p;
      }
    }
  }
  ...
  function buildProps(node, context, props = node.props, ssr = false) {
    ...
    const analyzePatchFlag = ({ key, value }) => {
      if (isStaticExp(key)) {
        ...
        if (
          ...
          ((value.type === 4 /* SIMPLE_EXPRESSION */ || value.type === 8) /* COMPOUND_EXPRESSION */ && getConstantType(value, context) > 0)
        ) {
    ...
    for (let i = 0; i < props.length; i++) {
      ...
      if (prop.type === 6 /* ATTRIBUTE */) {
        ...
        if (name === "is" || (isVBind && isStatibookgOf(arg, "is") && (isComponentTag(tag) || false))) {
          continue;
        }
    ...
    return {
      props: [
        createObjectProperty(
          ...
          exp ? createCallExpression(context.helperString(TO_DISPLAY_STRING), [exp], loc) : createSimpleExpression("", true)
        ),
      ],
    };
  };
